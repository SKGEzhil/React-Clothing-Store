...
const FilterBar: React.FC = () => {
    ...
    const applyFilters = () => {
        const filteredProducts = products.filter(product => {
            ...
            return isSizeMatch() && isCategoryMatch() && isPriceMatch && isRatingMatch;
        });
    ...
    return (
        <div className="relative">
            <button
                ...
                className={`md:hidden flex absolute z-30 left-0 top-20 transform -translate-y-1/2 py-4 px-1 bg-gray-100 shadow border-gray-400 rounded-md focus:outline-none transition-transform duration-300 ${
            ...
            <div
                className={`w-64 p-4 rounded-xl ml-4 fixed top-20 left-0 bg-white shadow-xl transition-transform duration-300 transform ${
                ...
                <div className="mb-4">
                    ...
                    <ul>
                        {categories.map(category => (
                            <li key={category.title.toLowerCase()} className="mb-2">
                                <label className="inline-flex items-center">
                                    <input
                                        type="checkbox"
                                        className="form-checkbox h-5 w-5 text-red-500"
                                        checked={selectedCategories.includes(category.title.toLowerCase())}
                                        onChange={() => handleCategoryChange(category.title.toLowerCase())}
                                    />
